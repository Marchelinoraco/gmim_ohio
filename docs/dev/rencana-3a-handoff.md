...
- Kalau index diperkuat lebih dulu (tetap NULL di baris lama): pembuatan index NULLS NOT DISTINCT akan **gagal langsung** — ada **16 grup baris** yang saat ini berbagi `(template_id, service_date, kolom_id) = (NULL, tanggal-sama, NULL)` untuk lima kategori non-kolom (tiap tanggal generate biasanya punya 5 kategori non-kolom + 1 grup 4-baris kolom; kelima yang non-kolom itulah yang duplikat begitu NULL berhenti dianggap distinct). Migrasi akan ditolak Postgres sampai backfill dilakukan.
